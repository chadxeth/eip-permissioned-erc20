...
describe("TransferOracle", () => {
  ...
  const getDefaultProof = (): string => {
    // Minimal valid encoding for the tuple (uint256[2], uint256[2][2], uint256[2])
    const defaultVal = 1n; // Use bigint for uint256 values
    const a: [bigint, bigint] = [defaultVal, defaultVal];
    const b: [[bigint, bigint], [bigint, bigint]] = [[defaultVal, defaultVal], [defaultVal, defaultVal]];
    const c: [bigint, bigint] = [defaultVal, defaultVal];
    // Define the tuple type string
    const ProofTupleType = "tuple(uint256[2] a, uint256[2][2] b, uint256[2] c)";
    // Encode the single tuple structure
    return ethers.AbiCoder.defaultAbiCoder().encode([ProofTupleType], [[a, b, c]]);
  };

  // Helper function to create public inputs bytes matching contract decoding
  const getCorrectDefaultPublicInputs = (root: string, senderHash: string, recipientHash: string, minAmtScaled: bigint, maxAmtScaled: bigint, expiry: number): string => {
    ...
    const inputs: bigint[] = [
      ...
      minAmtScaled,
      maxAmtScaled,
      BigInt(dummyCurrencyHashBytes32),
      BigInt(expiry)
    ];
    return ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [inputs]);
  };
  ...
  async function deployTransferOracleFixture() {
    ...
    const mockVerifierFactory = new MockGroth16Verifier__factory(_deployer);
  ...
  describe("2.1. Constructor & Initial State", () => {
    ...
    it("Should revert if token address is address(0)", async () => {
      ...
      const tempMockVerifierFactory = new MockGroth16Verifier__factory(deployer);
  ...
  describe("2.2. approveTransfer", () => {
    ...
    beforeEach(async () => {
      ...
      // Pass the valid bytes32 hex strings to the helper
      defaultPublicInputsBytes = getCorrectDefaultPublicInputs(rootHashBytes32, senderHashBytes32, recipientHashBytes32, minAmtScaled, maxAmtScaled, expiryTimestamp);
    ...
    describe("ZK Proof & Public Input Handling", () => {
      ...
      it("Should revert on input consistency check failure (minAmt)", async () => {
          ...
          const inconsistentPublicInputs = getCorrectDefaultPublicInputs(rootHashBytes32, senderHashBytes32, recipientHashBytes32, wrongMinScaled, maxAmtScaled, expiryTimestamp);
      ...
      it("Should revert on input consistency check failure (maxAmt)", async () => {
          ...
          const inconsistentPublicInputs = getCorrectDefaultPublicInputs(rootHashBytes32, senderHashBytes32, recipientHashBytes32, minAmtScaled, wrongMaxScaled, expiryTimestamp);
          await expect(transferOracle.connect(initialIssuer).approveTransfer(defaultApproval, defaultProofBytes, inconsistentPublicInputs))
              .to.be.revertedWithCustomError(transferOracle, "TransferOracle__InvalidPublicInputs");
      });
      ...
      it("Should revert on input consistency check failure (expiry)", async () => {
          ...
          const inconsistentPublicInputs = getCorrectDefaultPublicInputs(rootHashBytes32, senderHashBytes32, recipientHashBytes32, minAmtScaled, maxAmtScaled, wrongExpiry);
          await expect(transferOracle.connect(initialIssuer).approveTransfer(defaultApproval, defaultProofBytes, inconsistentPublicInputs))
              .to.be.revertedWithCustomError(transferOracle, "TransferOracle__InvalidPublicInputs");
      });
      ...
      it("Should revert if proofId in approval struct does not match calculated proofId", async () => {
          ...
          const consistentPublicInputs = getCorrectDefaultPublicInputs(rootHashBytes32, senderHashBytes32, recipientHashBytes32, minAmtScaled, maxAmtScaled, expiryTimestamp);
    ...
    describe("State Changes & Edge Cases", () => {
        it("Should allow multiple different approvals for the same (sender, recipient)", async () => {
            ...
            const inputs2 = getCorrectDefaultPublicInputs(root2, senderHashBytes32, recipientHashBytes32, BigInt(approval2.minAmt) * BigInt(1000), BigInt(approval2.maxAmt) * BigInt(1000), expiryTimestamp);
  ...
  describe("2.3. canTransfer", () => {
    ...
    let mockVerifierInstance: MockGroth16Verifier;
    ...
    beforeEach(async () => {
        ...
        inputsWide = getCorrectDefaultPublicInputs(rootW, shW, rhW, BigInt(approvalWide.minAmt)*BigInt(1000), BigInt(approvalWide.maxAmt)*BigInt(1000), futureExpiry);
        ...
        inputsTight = getCorrectDefaultPublicInputs(rootT, shW, rhW, BigInt(approvalTight.minAmt)*BigInt(1000), BigInt(approvalTight.maxAmt)*BigInt(1000), futureExpiry);
        ...
        inputsExpired = getCorrectDefaultPublicInputs(rootE, shE, rhE, BigInt(approvalExpired.minAmt)*BigInt(1000), BigInt(approvalExpired.maxAmt)*BigInt(1000), expiryForExpiredSetup);
  ...
  describe("2.4. View Functions", () => {
    ...
    describe("getApprovalCount(address sender, address recipient)", () => {
       ...
       beforeEach(async () => {
          ...
          const inputs1 = getCorrectDefaultPublicInputs(root1, sh1, rh1, BigInt(approval1.minAmt)*BigInt(1000), BigInt(approval1.maxAmt)*BigInt(1000), expiry);
          ...
          const inputs2 = getCorrectDefaultPublicInputs(root2, sh1, rh1, BigInt(approval2.minAmt)*BigInt(1000), BigInt(approval2.maxAmt)*BigInt(1000), expiry);
    ...
    describe("isProofUsed(bytes32 proofId)", () => {
        ...
        beforeEach(async () => {
            ...
            const inputs = getCorrectDefaultPublicInputs(root, sh, rh, BigInt(approval.minAmt)*BigInt(1000), BigInt(approval.maxAmt)*BigInt(1000), expiry);
