...
contract PermissionedERC20 is ERC20, Ownable {
    ...
    function _update(address from, address to, uint256 amount) internal virtual override {
        if (from != address(0) && to != address(0)) {
            ...
            // For mints (from == 0) or burns (to == 0), no oracle check is needed.
            super._update(from, to, amount); // Call super._update first
            emit TransferValidated(proofId); // Emit event after successful super._update
        }
        else {
             super._update(from, to, amount); // For mints/burns, just call super
        }
    }
    ...
    // Note: A direct `burn(uint256 amount)` function calling `_burn(msg.sender, amount)`
    // could also be added if users should be able to burn their own tokens without owner intervention.
    // However, the brief only specified owner-only mint/burn, implying `burnFrom` is sufficient.
} 